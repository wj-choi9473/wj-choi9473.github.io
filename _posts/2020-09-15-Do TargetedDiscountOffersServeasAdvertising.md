---
title: "각 고객들을 타게팅한 할인쿠폰이 광고의 효과도 있을까?"
categories: 
  - 논문
use_math: true
layout: single
...
마케팅 모형론 첫시간에 case study로 다뤘던 promotional e-mail에 과 같이 이메일로 보낸 프로모션세일 대한 연구를 한 페이퍼입니다.
첫시간에는 이메일을 얼마나 보내는게 좋은지에 대한 고민이었다면
여기 저자들은 프로모션 세일을 오퍼하는 이메일이 과연 어떤 효과가 있는지를 보는 페이퍼 입니다.
...
첫시간 이메일 케이스 스터디에선 reverse causality 와 sample selection 문제가 있었는데 이 저자들은 그런 문제들을 어떻게 해결했는지 보도록 하겠습니다.
...
우선 이메일 마케팅은 보통 일반적으로 가격차별을 위해 할인쿠폰을 발급하기 위한 수단으로 쓰일수 있고요, 저자들은 여기에 더해서 광고적인 효과도 있다고 합니다. 즉, 다음 구매에 이 회사의 제품으로 구매하게 하는 효과를 볼 수 가 있습니다.   

하지만 이메일 마케팅 특성상 selection bias가 생길수있는데요,  왜냐하면 전체 집단을 보려하면 이메일 마케팅 수신 동의를 한 소비자와 안한 소비자의 특성이 다를 수 있고 , 이메일 수신을 동의한 소비자들 사이에서도 자주 구매하는지 등 행동 특성에 따라 타게팅을 다르게 할 수 있기 때문입니다.   


## Empirical Setting and Data

<img src = "/assets/img/blog/session2/1.png" >
저자들은 이문제를 전체적인 소비자를 본것이 아닌 마케팅 수신메일을 동의한 소비자들중 targeted 된 소비자들을 랜덤하게 홍보메일을 보내는 그룹과 안보내는 그룹으로 나누어 70개의 실험(즉, 프로모션 캠페인)으로 해결하였습니다.


<img src = "/assets/img/blog/session2/2.png" >
전체 샘플 집단에서 옵트인 한 그룹만을 가져 온 이유는
전체 샘플 집단과 옵트인한 그룹을 비교해 봤을때 이메일을 받은 정도는 비슷하지만 표에 나와있는 특성들, expenditure and transactions per year 과 같은 특성들이 통계적으로 다르다고 합니다. 즉, 저자들은 선택 편의의 문제를 해결 하며 두 집단을 비교하며 확실히 합니다. 


<img src = "/assets/img/blog/session2/3.png" >
데이터를 더 자세히 보면, 프로모션 기간은 보통 이주에서 한달 사이에 다 끝났고 총 122개의 프로모션 캠페인중 76개의 캠페인이 treatment 와 control을 준 캠페인이고 그 중에서 6개의 캠페인은 참가된 개인의 수가 100명이 안되어 제외하고 100명 이상을 참여시킨 캠페인 총 70개의 캠페인 데이터를 가지고 분석을 진행합니다.
또한 피규어 2를 보시면 각 개인이 프로모션 안에 여러번 참여하게 될 수도 있다는 것을 보여줍니다.



<img src = "/assets/img/blog/session2/4.png" >
...
두번째 점검으로 각 캠페인에 대한 dummy indicator를 넣어 70개 캠페인의 고졍효과로 회귀분석을 실행했을시에도 인구통계나 과거transaction이 통계적으로 유의한 계수는 없는걸 보여줘 실험에 참가한 각 개인은 무작위 할당이라는 기업의 주장(저자들의 가정)을 뒷받침 합니다.


<img src = "/assets/img/blog/session2/5.png" >  
...
이러한 데이터 분석을 한 이후 Empirical Approach단계로 갑니다.
...
피규어4를 보시면 70개의 실험에서 treatment propensity가 0.31 에서 0.97까지 크게 다른데 회사의 의사결정 때문일것이라고 합니다.
...
Empirical appporach 에서 접근한 방식대로 샘플데이터에 대해서 회귀식을 돌리는데요 
...
<img src = "/assets/img/blog/session2/7.png" > 
...
<img src = "/assets/img/blog/session2/8.png" > 
...
<img src = "/assets/img/blog/session2/9.png" > 
...
<img src = "/assets/img/blog/session2/10.png" > 
...
<img src = "/assets/img/blog/session2/11.png" >
...
<img src = "/assets/img/blog/session2/12.png" > 
...
<img src = "/assets/img/blog/session2/13.png" > 
